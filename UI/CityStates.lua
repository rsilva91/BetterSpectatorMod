...
include("TeamSupport");
print("CityState Panel for Better Spectator Mod")
...
function GetCityStatesMetNum()
	...
	for _,kCityState in pairs(m_kCityStates) do
		if kCityState.isHasMet or bspec == true then
			total = total + 1;
		end
	end
	return total;
...
function OpenOverview( iPlayer:number )	
	--print("OpenOverview)", Game.GetLocalPlayer())
...
function AddCityStateRow( kCityState:table )
	--print("AddCityStateRow()", Game.GetLocalPlayer())
...
-- ===========================================================================
function ViewList()
	--print("ViewList()", Game.GetLocalPlayer())
	local localPlayerID = Game.GetLocalPlayer();
	local bspec = false
	if (Game:GetProperty("SPEC_NUM") ~= nil) then
		for k = 1, Game:GetProperty("SPEC_NUM") do
...
function ViewCityState( iPlayer:number )
	--print("ViewCityState()", Game.GetLocalPlayer())
...
-- ===========================================================================
function GetData()
	--print("GetData()", Game.GetLocalPlayer())
	local bspec = false
	if (Game:GetProperty("SPEC_NUM") ~= nil) then
		for k = 1, Game:GetProperty("SPEC_NUM") do
			if ( Game:GetProperty("SPEC_ID_"..k)~= nil) then
	...
	for i, pPlayer in ipairs(PlayerManager.GetAliveMinors()) do
		...
		if iPlayer ~= pLocalPlayer:GetID() and isCanReceiveInfluence then
			...
			--if Game.GetLocalPlayer() == spec_ID then
			--	kCityState.isHasMet = true
			--end
...
function OnInit(isReload:boolean)
	--print("OnInit()", Game.GetLocalPlayer())
	if isReload then
...
function OnUpdateUI( type:number, tag:string, iData1:number, iData2:number, strData1:string)
	--print("OnUpdateUI()", Game.GetLocalPlayer())
...
function OnLocalPlayerTurnBegin()
	--print("OnLocalPlayerTurnBegin()", Game.GetLocalPlayer())
	local bspec = false
	if (Game:GetProperty("SPEC_NUM") ~= nil) then
		for k = 1, Game:GetProperty("SPEC_NUM") do
			if ( Game:GetProperty("SPEC_ID_"..k)~= nil) then
	...
	if ContextPtr:IsHidden() == false or bspec == true then
		Refresh();
	end
end
...
-- ===========================================================================
function Initialize()
	--print("Initialize()", Game.GetLocalPlayer())
